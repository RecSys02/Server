...
public class ChatbotService {
    ...
    public Flux<ServerSentEvent<String>> getChatStream(ChatbotRequest request) {
        return authService.currentUserId()
                .flatMapMany(userId -> processChatStream(userId, request, userContextService.getOrFetch(userId)));
    }

    public Flux<ServerSentEvent<String>> getChatStreamWithFreshContext(ChatbotRequest request) {
        return authService.currentUserId()
                .flatMapMany(userId -> processChatStream(userId, request, userContextService.fetch(userId)));
    }

    private Flux<ServerSentEvent<String>> processChatStream(Long userId, ChatbotRequest request, Mono<UserContextDto> userContextMono) {
        return Mono.zip(
                userContextMono,
